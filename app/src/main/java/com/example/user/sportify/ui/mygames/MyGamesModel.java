...
public class MyGamesModel {
    ...
    private Call<BaseServerAnswer<String>> callCancelGameApi(String token, int cancel, String gameId) {
...
//                callback.onSendGamesData();
    ...
    class CancelGameTask extends AsyncTask<Void, Void, Void> {
        ...
        @Override
        protected Void doInBackground(Void... voids) {
            callCancelGameApi(token, cancel, gameId).enqueue(new Callback<BaseServerAnswer<String>>() {
                @Override
                public void onResponse(@NonNull Call<BaseServerAnswer<String>> call, @NonNull Response<BaseServerAnswer<String>> response) {
                    if (callback != null && response.body() != null) {
                        callback.onSendResponse(Objects.requireNonNull(response.body()).getSuccess().toString());
