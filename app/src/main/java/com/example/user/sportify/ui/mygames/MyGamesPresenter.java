...
class MyGamesPresenter extends MvpBasePresenter<MyGamesView> {
		...
		new android.os.Handler().postDelayed(() -> {
			myGamesModel.getGamesParticipant(gamesParticipantData -> {
				if (gamesParticipantData != null) {
					...
					ifViewAttached(view -> view.initGamesRecyclerView(new LinearLayoutManager(
						context), new GamesAdapter(context, (view1, position, game) -> {
						if (game.getIsCanceled() != 1) {
							view.startNewActivity(new Intent(
								context,
	...
	private void organizerType() {
		new android.os.Handler().postDelayed(() -> {
			ifViewAttached(view -> view.initGamesRecyclerView(
				new LinearLayoutManager(context),
				new GamesAdapter(context, (view1, position, game) -> {
					if (game.getIsCanceled() != 1) {
						view.startNewActivity(new Intent(
							context,
							ConcretGameActivity.class).putExtra(EXTRA_GAME, game).putExtra(
					...
					(position, gameId, game) -> {
						view.startNewActivity(new Intent(context, CreateGame.class).putExtra(
							EXTRA_GAME,
							game));
					})));
