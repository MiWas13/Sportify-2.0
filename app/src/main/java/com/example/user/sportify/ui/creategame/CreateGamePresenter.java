...
class CreateGamePresenter extends MvpBasePresenter<CreateGameView> {
	...
	void onDataButtonClicked() {
		...
		DatePickerDialog datePickerDialog = new DatePickerDialog(
			context,
			new NewGameDatePickerListener(this),
	...
	void onDataSet(int year, int month, int day) {
		...
		TimePickerDialog timePickerDialog = new TimePickerDialog(
			context,
			new NewGameTimePickerListener(this, year, month, day),
	...
	void onTimeSet(int year, int month, int day, int hours, int minutes) {
		...
		ifViewAttached(view -> view.setDateFieldValue(getCurrentDate(
			year,
			month + 1,
			day,
			hours,
	...
	void onLoadImage(int requestCode, int resultCode, Intent imageReturnedIntent) {
		...
		switch (requestCode) {
			case GALLERY_REQUEST:
				if (resultCode == RESULT_OK) {
					...
					ifViewAttached(view -> {
						view.setLocationImage(compressBitmap(getRealPathFromURI(
							context,
							imageReturnedIntent.getData()), 360, 162));
					});
		...
		this.file = FileUtils.getFile(
			context,
			Uri.fromFile(fromBitmapToFile(compressBitmap(getRealPathFromURI(
				context,
				imageReturnedIntent.getData()), 360, 162))));
			...
			new SearchManager.SuggestListener() {
				...
				public void onSuggestResponse(@NonNull List<SuggestItem> suggest) {
					...
					for (int i = 0; i < Math.min(RESULT_NUMBER_LIMIT, suggest.size()); i++) {
						if (Objects.requireNonNull(suggest.get(i).getDisplayText()).contains(
							"Россия")) {
	...
	void dontHavePermission() {
		ifViewAttached(view -> view.showPermissionSnack(
			"У нас нет разрешения, дайте его в настройках, пожалуйста"));
	}
	...
	void fieldChanged(String inputText, int fieldType) {
		switch (fieldType) {
			case NEW_MAX_PEOPLE_QUANTITY:
				try {
					isMaxQuantityValid = Integer.valueOf(inputText) >= 2 && Integer.valueOf(
						inputText) < 30;
				} catch (Exception e) {
				...
				new android.os.Handler().postDelayed(() -> {
					if (!isChanging) {
						createGameModel.createGame(
							...
							file,
							createGameModel.getSessionData().authToken,
							categoryId,
							description,
							date,
							time,
